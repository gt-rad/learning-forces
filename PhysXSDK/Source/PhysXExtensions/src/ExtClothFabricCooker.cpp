...
namespace
{
		...
		const PxBoundedData& triangles, const PxBoundedData& quads)
	{
		...
		qIt = PxMakeIterator((const T*)quads.data, quads.stride);
	...
	struct ConstraintGraphColorCount
	{
		ConstraintGraphColorCount(int cid, int count) 
			: constraint((PxU32)cid), colorCount((PxU32)count) {}
...
{	
	...
	for(PxU32 i=0; i<numConstraints; ++i)
	{
        ...
        // START ADDED BY NSF-DRESSING
        if (mPhases[colors[i]].phaseType == PxClothFabricPhaseType::eBENDING) {
            //mRestvalues[index] = reinterpret_cast<
            //const PxVec3&>(diff).magnitude() * 1.3;
        }
        // END ADDED BY NSF-DRESSING
	} 
