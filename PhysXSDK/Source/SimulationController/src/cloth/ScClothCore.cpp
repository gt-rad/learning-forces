...
void Sc::ClothCore::initLowLevel(const PxTransform& globalPose, const PxClothParticle* particles)
{
	...
	mPhaseConfigs = reinterpret_cast<cloth::PhaseConfig*>(PX_ALLOC(nbPhases * sizeof(cloth::PhaseConfig), PX_DEBUG_EXP("cloth::PhaseConfig")));
...
void Sc::ClothCore::setParticles(const PxClothParticle* currentParticles, const PxClothParticle* previousParticles)
{
	if (currentParticles)
	{
		...
		if((PxClothParticle*)particlesRange.begin() != currentParticles)
	...
	if (previousParticles)
	{
		...
		if((PxClothParticle*)particlesRange.begin() != previousParticles)
...
// START ADDED BY NSF-DRESSING
void Sc::ClothCore::setSelfFrictionCoefficient(PxReal frictionCoefficient)
{
    mLowLevelCloth->setSelfFriction(frictionCoefficient);
}


PxReal Sc::ClothCore::getSelfFrictionCoefficient() const
{
    return mLowLevelCloth->getSelfFriction();
}

void Sc::ClothCore::setStiffnessPower(uint32_t stiffpower)
{
    mLowLevelCloth->setStiffnessPower(stiffpower);
}


uint32_t Sc::ClothCore::getStiffnessPower() const
{
    return mLowLevelCloth->getStiffnessPower();
}
// END ADDED BY NSF-DRESSING
...
void Sc::ClothCore::setClothFlag(PxClothFlag::Enum flag, bool val)
{
	val ? mClothFlags |= flag : mClothFlags &= ~flag;

	switch(flag)
	{
	case PxClothFlag::eSWEPT_CONTACT:
		mLowLevelCloth->enableContinuousCollision(val);
		break;
	case PxClothFlag::eGPU:
		if (getSim() && val != (mLowLevelCloth->getFactory().getPlatform() != cloth::Factory::CPU))
			getSim()->reinsert();
		break;
	case PxClothFlag::eSCENE_COLLISION:
		if(!val && getSim())
			getSim()->clearCollisionShapes();
		break;
	case PxClothFlag::eCOUNT:
	default:
		break;
	}
}
...
void Sc::ClothCore::getParticleData(PxClothParticleData& data)
{
	if(data.getDataAccessFlags() & PxDataAccessFlag::eDEVICE)
	{
		...
		data.particles = (PxClothParticle*)particles.mCurrent;
		data.previousParticles = (PxClothParticle*)particles.mPrevious;
	} 
	else
	{
...
// START ADDED BY NSF-DRESSING
PxVec3 Sc::ClothCore::getAccumulatedCollisionForce() const {
    return mLowLevelCloth->getAccumulatedCollisionForces();
}

float* Sc::ClothCore::getContactForces() const {
    return mLowLevelCloth->getContactForces();
}

float* Sc::ClothCore::getFrictionForces() const {
    return mLowLevelCloth->getFrictionForces();
}

void Sc::ClothCore::setSolverFlags(char flag) const {
    mLowLevelCloth->setSolverFlags(flag);
}
// END ADDED BY NSF-DRESSING
