...
{
	MappedRange(T* first, T* last, const Cloth& cloth, 
		void (Cloth::*lock)() const, void (Cloth::*unlock)() const)
...
public:
    ...
    // START ADDED BY NSF-DRESSING
    virtual PxVec3 getAccumulatedCollisionForces() const = 0;
    ...
    // START ADDED BY NSF-DRESSING
    // self friction
    virtual void setSelfFriction(float) = 0;
    virtual float getSelfFriction() const = 0;
    
    // stiffness power
    virtual void setStiffnessPower(uint32_t) = 0;
	...
	virtual void setVirtualParticles(Range<const uint32_t[4]> indices, 
		Range<const PxVec3> weights) = 0;
    ...
    // START ADDED BY NSF-DRESSING
    virtual void setSolverFlags(char) = 0;
    // END ADDED BY NSF-DRESSING
};
