...
class SwCloth
{
	...
	struct SwContextLock { SwContextLock(const SwFactory&) {} };
...
public:
	...
	Vec4fAlignedVector mPrevParticles;
        ...
        // current and previous-iteration particle positions before collision step
        // used to extract collision forces
        Vec4fAlignedVector mCurParticlesPreCollision;
        Vec4fAlignedVector mPrevParticlesPreCollision;
    
        // accumulated collision force
        PxVec3 mAccumCollisionForce;
    
        // contact force and friction force
        float* mContactForces;
        float* mFrictionForces;
    ...
    // START ADDED BY NSF-DRESSING
    char mSolverFlags;  // flags to decide which procedures to use
    // END ADDED BY NSF-DRESSING
