...
cloth::SwClothData::SwClothData( SwCloth& cloth, const SwFabric& fabric )
{
	...
	// START ADDED BY NSF-DRESSING
	mCurParticlesPreCollision = array(cloth.mCurParticlesPreCollision.front());
	mPrevParticlesPreCollision = array(cloth.mPrevParticlesPreCollision.front());
    
	mContactForces = cloth.mContactForces;
	mFrictionForces = cloth.mFrictionForces;
	// END ADDED BY NSF-DRESSING
	...
	float stiffnessExponent = cloth.mStiffnessFrequency * 
		cloth.mPrevIterDt * 0.69314718055994531f; // logf(2.0f);
	...
	mStartMotionConstraints = cloth.mMotionConstraints.mStart.size() ? 
	...
	mStartSeparationConstraints = cloth.mSeparationConstraints.mStart.size() ? 
		array(cloth.mSeparationConstraints.mStart.front()) : 0;
	mTargetSeparationConstraints = !cloth.mSeparationConstraints.mTarget.empty() ? 
		array(cloth.mSeparationConstraints.mTarget.front()) : 0;

	mParticleAccelerations = cloth.mParticleAccelerations.size() ? 
		array(cloth.mParticleAccelerations.front()) : 0;

	mStartCollisionSpheres = cloth.mStartCollisionSpheres.empty() ? 
		0 : array(cloth.mStartCollisionSpheres.front());	
	mTargetCollisionSpheres = cloth.mTargetCollisionSpheres.empty() ? 
		mStartCollisionSpheres : array(cloth.mTargetCollisionSpheres.front());
	...
	mStartCollisionPlanes = cloth.mStartCollisionPlanes.empty() ? 
		0 : array(cloth.mStartCollisionPlanes.front());
	mTargetCollisionPlanes = cloth.mTargetCollisionPlanes.empty() ? 
		mStartCollisionPlanes : array(cloth.mTargetCollisionPlanes.front());
	...
	mStartCollisionTriangles = cloth.mStartCollisionTriangles.empty() ? 
		0 : array(cloth.mStartCollisionTriangles.front());
	mTargetCollisionTriangles = cloth.mTargetCollisionTriangles.empty() ? 
		mStartCollisionTriangles : array(cloth.mTargetCollisionTriangles.front());
	mNumTriangles = uint32_t(cloth.mStartCollisionTriangles.size()) / 3;

	mVirtualParticlesBegin = cloth.mVirtualParticleIndices.empty() ? 
		0 :	array(cloth.mVirtualParticleIndices.front());
	mVirtualParticlesEnd = mVirtualParticlesBegin 
		+ 4 * cloth.mVirtualParticleIndices.size();
	mVirtualParticleWeights = cloth.mVirtualParticleWeights.empty() ?
		0 : array(cloth.mVirtualParticleWeights.front());
	...
	// START ADDED BY NSF-DRESSING
	mSelfFrictionScale = cloth.mSelfFriction;
	mStiffPower = cloth.mStiffPower;
	// END ADDED BY NSF-DRESSING
	...
	mSelfCollisionIndices = cloth.mSelfCollisionIndices.empty() ? 
		0 : cloth.mSelfCollisionIndices.begin();
	mNumSelfCollisionIndices = mSelfCollisionIndices ? 
		cloth.mSelfCollisionIndices.size() : mNumParticles;

	mRestPositions = cloth.mRestPositions.size() ? 
		array(cloth.mRestPositions.front()) : 0;
	...
	// START ADDED BY NSF-DRESSING
	// initialize to zero
	mAccumulatedCollisionForce[0] = 0;
	mAccumulatedCollisionForce[1] = 0;
	mAccumulatedCollisionForce[2] = 0;
	// END ADDED BY NSF-DRESSING
}
...
void cloth::SwClothData::reconcile(SwCloth& cloth) const
{
	...
	// START ADDED BY NSF-DRESSING
	cloth.mAccumCollisionForce.x = mAccumulatedCollisionForce[0];
	cloth.mAccumCollisionForce.y = mAccumulatedCollisionForce[1];
	cloth.mAccumCollisionForce.z = mAccumulatedCollisionForce[2];    
	// START ADDED BY NSF-DRESSING
}
...
void cloth::SwClothData::verify() const
{
	...
	PX_ASSERT(!mNumCapsules || mNumSpheres > *shdfnd::maxElement(
		&mCapsuleIndices->first, &(mCapsuleIndices+mNumCapsules)->first));

	PX_ASSERT(!mNumConvexes || (1u << mNumPlanes) - 1 >=
		*shdfnd::maxElement(mConvexMasks, mConvexMasks+mNumConvexes));
