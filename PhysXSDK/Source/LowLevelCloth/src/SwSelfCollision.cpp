...
namespace
{
	...
	void radixSort(const uint32_t* first, const uint32_t* last, uint16_t* out)
	{
		...
		uint16_t* __restrict histograms[] = { buffer, 
			buffer + 256, buffer + 512, buffer + 768 };
	...
	{
		return PxMin(cloth.mSelfCollisionDistance, 
			cloth.mSelfCollisionStiffness) > 0.0f;
	}

	bool isSelfCollisionEnabled(const cloth::SwCloth& cloth)
	{
		return PxMin(cloth.mSelfCollisionDistance,
			-cloth.mSelfCollisionLogStiffness) > 0.0f;
	}

	inline uint32_t align2(uint32_t x) { return (x+1) & ~1; }
...
void cloth::SwSelfCollision<Simd4f>::operator()()
{
	...
	const Simd4f* particles = reinterpret_cast<
		const Simd4f*>(mClothData.mCurParticles);
...
{
	uint32_t numIndices = cloth.mSelfCollisionIndices.empty() ? 
		cloth.mCurParticles.size() : cloth.mSelfCollisionIndices.size();
...
size_t physx::cloth::SwSelfCollision<Simd4f>::getBufferSize( uint32_t numIndices )
{
	...
	return keysSize + indicesSize + PxMax(radixSize, 
...
void cloth::SwSelfCollision<Simd4f>::collideParticles(Simd4f& pos0, Simd4f& pos1, const Simd4f& pos0rest, const Simd4f& pos1rest, const Simd4f& prev0, const Simd4f& prev1, const Simd4f& frictionScale)
// END ADDED BY NSF-DRESSING
{
    ...
    Simd4f diffchange = pos0 - prev0 + prev1 - pos1;
    Simd4f normdiff = diff * rsqrt(distSqr);
    Simd4f normvelmag = dot3(normdiff, diffchange);
    Simd4f relvel = diffchange - normdiff * normvelmag;
    Simd4f pendepth = sqrt(mCollisionSquareDistance) - sqrt(distSqr);
    Simd4f j = max(-frictionScale*pendepth*rsqrt(dot3(relvel, relvel)), simd4f(-1))&sMaskXYZ;
    Simd4f w0 = splat<3>(pos0);
    Simd4f w1 = splat<3>(pos1);
    Simd4f scale = mStiffness * recip(sEpsilon + w0 + w1);
    
    Simd4f frictionimpulse = (scale * j * relvel)&sMaskXYZ;
    
    /*Simd4f show = j;
    const float* showarray = array(show);
    printf("show %f, %f, %f, %f\n", showarray[0], showarray[1], showarray[2], showarray[3]);*/
    
    pos0 = pos0 + frictionimpulse * w0;
    pos1 = pos1 - frictionimpulse * w1;
...
template <typename Simd4f>
template <bool useRestParticles>
void cloth::SwSelfCollision<Simd4f>::collideParticles(const uint32_t* keys, 
	uint16_t firstColumnSize, const uint16_t* indices, uint32_t collisionDistance)
{
	Simd4f* __restrict particles = reinterpret_cast<Simd4f*>(mClothData.mCurParticles);
    // START ADDED BY NSF-DRESSING
    Simd4f* __restrict prevparticles = reinterpret_cast<Simd4f*>(mClothData.mPrevParticles);
    // END ADDED BY NSF-DRESSING
	Simd4f* __restrict restParticles = useRestParticles?reinterpret_cast<Simd4f*>(mClothData.mRestPositions):particles;
    ...
    // START ADDED BY NSF-DRESSING
    const Simd4f frictionScale = simd4f(mClothData.mSelfFrictionScale);
    // END ADDED BY NSF-DRESSING
	...
	for(; iIt != iEnd; ++iIt, ++kFirst[0])
	{
        ...
        // START ADDED BY NSF-DRESSING
        Simd4f prevParticle = prevparticles[*iIt];
        // END ADDED BY NSF-DRESSING
		...
		for(jIt = iIt + 1; jIt != jEnd; ++jIt)
            // START ADDED BY NSF-DRESSING
			collideParticles<useRestParticles>(particle, particles[*jIt], restParticle, restParticles[*jIt], prevParticle, prevparticles[*jIt], frictionScale);
            // END ADDED BY NSF-DRESSING
