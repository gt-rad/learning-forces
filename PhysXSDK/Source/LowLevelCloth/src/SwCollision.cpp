...
const Simd4f cloth::SwCollision<Simd4f>::sSkeletonWidth = simd4f(0.95f);
...
#elif NVMATH_VMX128 || NVMATH_ALTIVEC
const Simd4i cloth::Gather<Simd4i>::sPack = 
	simd4i(0x03030303, 0x07070707, 0x0b0b0b0b, 0x0f0f0f0f);
const Simd4i cloth::Gather<Simd4i>::sOffset = simd4i(0x00010203);
...
#elif NVMATH_NEON
const Simd4i cloth::Gather<Simd4i>::sPack = 
	simd4i(0x00000000, 0x04040404, 0x08080808, 0x0c0c0c0c);
...
#if defined(PX_PROFILE) || defined(PX_DEBUG)
	template <typename Simd4f>
...
namespace physx
{
	...
	{
		template <typename Simd4f>
		BoundingBox<Simd4f> expandBounds(
			const BoundingBox<Simd4f>& bbox, 
			const SphereData* sIt, const SphereData* sEnd)
		{
...
namespace
{
	...
	void generateCones(cloth::ConeData* dst, const cloth::SphereData* sourceSpheres, 
		const cloth::IndexPair* capsuleIndices, uint32_t numCones)
	{
		cloth::ConeData* cIt = dst;
		for(const cloth::IndexPair *iIt = capsuleIndices, *iEnd = iIt 
			+ numCones; iIt != iEnd; ++iIt, ++cIt)
		{
			PxVec4 first = reinterpret_cast<
				const PxVec4&>(sourceSpheres[iIt->first]);
			PxVec4 second = reinterpret_cast<
				const PxVec4&>(sourceSpheres[iIt->second]);
	...
	template <typename Simd4f, typename SrcIterator>
	void generateTriangles(cloth::TriangleData* dIt, 
		const SrcIterator& src, uint32_t count)
	{
		// have to copy out iterator to ensure alignment is maintained
...
template <typename Simd4f>
cloth::SwCollision<Simd4f>::SwCollision(SwClothData& clothData, SwKernelAllocator& alloc, PxProfileZone* profiler)
: mClothData(clothData), mAllocator(alloc), mProfiler(profiler)
{
	...
	if(mClothData.mEnableContinuousCollision || mClothData.mFrictionScale > 0.0f)
	{
		...
		generateCones(mPrevData.mCones, mPrevData.mSpheres, 
			clothData.mCapsuleIndices, clothData.mNumCapsules);
	}
}
...
// START ADDED BY NSF-DRESSING
template <typename Simd4f>
void cloth::SwCollision<Simd4f>::recordPreCollisionPositions() {
    ...
    float* __restrict curPreColIt = mClothData.mCurParticlesPreCollision;
    float* __restrict prevPreColIt = mClothData.mPrevParticlesPreCollision;
    
    Simd4f curPos[4], prevPos[4];
    
    for (; curIt < curEnd; curIt += 16, prevIt += 16, curPreColIt += 16, prevPreColIt += 16) {
        curPos[0] = loadAligned(curIt,  0);
        curPos[1] = loadAligned(curIt, 16);
        curPos[2] = loadAligned(curIt, 32);
        curPos[3] = loadAligned(curIt, 48);
        storeAligned(curPreColIt,  0, curPos[0]);
        storeAligned(curPreColIt, 16, curPos[1]);
        storeAligned(curPreColIt, 32, curPos[2]);
        storeAligned(curPreColIt, 48, curPos[3]);
        ...
        storeAligned(prevPreColIt,  0, prevPos[0]);
        storeAligned(prevPreColIt, 16, prevPos[1]);
        storeAligned(prevPreColIt, 32, prevPos[2]);
        storeAligned(prevPreColIt, 48, prevPos[3]);
    }
}

template <typename Simd4f>
void cloth::SwCollision<Simd4f>::retrieveCollisionPositions() {
    float* __restrict curIt = mClothData.mCurParticles;
    float* __restrict curEnd = curIt + mClothData.mNumParticles * 4;
    float* __restrict prevIt = mClothData.mPrevParticles;
    float* __restrict curPreColIt = mClothData.mCurParticlesPreCollision;
    ...
    Simd4f curPos[4], prevPos[4], curPosPreCol[4], prevPosPreCol[4];
    ...
    for (; curIt < curEnd; curIt += 16, prevIt += 16, curPreColIt += 16, prevPreColIt += 16) {
        ...
        curPos[2] = loadAligned(curIt, 32);
        curPos[3] = loadAligned(curIt, 48);
        curPosPreCol[0] = loadAligned(curPreColIt, 0);
        curPosPreCol[1] = loadAligned(curPreColIt, 16);
        curPosPreCol[2] = loadAligned(curPreColIt, 32);
        curPosPreCol[3] = loadAligned(curPreColIt, 48);
    
        prevPos[0] = loadAligned(prevIt,  0);
        prevPos[1] = loadAligned(prevIt, 16);
        prevPos[2] = loadAligned(prevIt, 32);
        prevPos[3] = loadAligned(prevIt, 48);
        prevPosPreCol[0] = loadAligned(prevPreColIt,  0);
        prevPosPreCol[1] = loadAligned(prevPreColIt, 16);
        prevPosPreCol[2] = loadAligned(prevPreColIt, 32);
        prevPosPreCol[3] = loadAligned(prevPreColIt, 48);
        
        for (int p = 0; p < 4; p++) {
            const float* curPos_array = array(curPos[p]);
            const float* curPosPreCol_array = array(curPosPreCol[p]);
            
            for (int axis = 0; axis < 3; axis++) {
                mClothData.mContactForces[(particle_index+p)*3 + axis] += (curPos_array[axis] - curPosPreCol_array[axis]);
            }
            
            const float* prevPos_array = array(prevPos[p]);
            const float* prevPosPreCol_array = array(prevPosPreCol[p]);
            for (int axis = 0; axis < 3; axis++) {
                mClothData.mFrictionForces[(particle_index+p)*3 + axis] += (prevPosPreCol_array[axis] - prevPos_array[axis]);
            }
        }
...
void cloth::SwCollision<Simd4f>::operator()(const IterationState<Simd4f>& state)
{
	...
	const Simd4f* targetSpheres = reinterpret_cast<
		const Simd4f*>(mClothData.mTargetCollisionSpheres);

	// generate sphere and cone collision data
	if (!lastIteration)
	{
		// interpolate spheres
		LerpIterator<Simd4f, const Simd4f*> pIter(reinterpret_cast<const Simd4f*>(
			mClothData.mStartCollisionSpheres), targetSpheres, state.getCurrentAlpha());
		generateSpheres(reinterpret_cast<Simd4f*>(
			mCurData.mSpheres), pIter, mClothData.mNumSpheres);
	}
	else
	{
		// otherwise use the target spheres directly
		generateSpheres(reinterpret_cast<Simd4f*>(
			mCurData.mSpheres), targetSpheres, mClothData.mNumSpheres);
	}

	// generate cones even if test below fails because 
	// continuous collision might need it in next iteration
	generateCones(mCurData.mCones, mCurData.mSpheres, 
		mClothData.mCapsuleIndices, mClothData.mNumCapsules);

	if(buildAcceleration())
	{
        // START ADDED BY NSF-DRESSING
		if(mClothData.mEnableContinuousCollision)
			collideContinuousParticles(state.mIterDt);
        // END ADDED BY NSF-DRESSING
...
{
	data.mSpheres = static_cast<SphereData*>(mAllocator.allocate(
		sizeof(SphereData) * mClothData.mNumSpheres));

	data.mCones = static_cast<ConeData*>(mAllocator.allocate(
		sizeof(ConeData) * mClothData.mNumCapsules));
}
...
namespace 
{
	...
	Simd4i andNotIsZero(
		const Simd4i& left, const Simd4i& right)
	{
...
bool cloth::SwCollision<Simd4f>::buildAcceleration()
{
	...
	BoundingBox<Simd4f> sphereBounds = expandBounds(emptyBounds<Simd4f>(), 
		mCurData.mSpheres, mCurData.mSpheres + mClothData.mNumSpheres);
	BoundingBox<Simd4f> particleBounds = loadBounds<Simd4f>(mClothData.mCurBounds);
	if(mClothData.mEnableContinuousCollision)
	{
		sphereBounds = expandBounds(sphereBounds, mPrevData.mSpheres, 
			mPrevData.mSpheres + mClothData.mNumSpheres);
		particleBounds = expandBounds(particleBounds, 
			loadBounds<Simd4f>(mClothData.mPrevBounds));
	}
	...
	PX_ASSERT(allTrue(((bounds.mLower * mGridScale + mGridBias) >= simd4f(0.0f)) | sMaskW)); //add parentheses for wiiu
	PX_ASSERT(allTrue(((bounds.mUpper * mGridScale + mGridBias) < simd4f(8.0f)) | sMaskW)); //add parentheses for wiiu
...
FORCE_INLINE typename cloth::SwCollision<Simd4f>::ShapeMask& 
	cloth::SwCollision<Simd4f>::ShapeMask::operator=(const ShapeMask& right)
{
	mCones = right.mCones;
...
template <typename Simd4f>
FORCE_INLINE typename cloth::SwCollision<Simd4f>::ShapeMask cloth::SwCollision<Simd4f>::getShapeMask(
	const Simd4f& position, const Simd4i* __restrict sphereGrid, 
...
template <typename Simd4f>
FORCE_INLINE typename cloth::SwCollision<Simd4f>::ShapeMask cloth::SwCollision<Simd4f>::getShapeMask(
	const Simd4f* __restrict positions) const
{
...
template <typename Simd4f>
FORCE_INLINE typename cloth::SwCollision<Simd4f>::ShapeMask cloth::SwCollision<Simd4f>::getShapeMask(
	const Simd4f* __restrict prevPos, const Simd4f* __restrict curPos) const
{
...
struct cloth::SwCollision<Simd4f>::ImpulseAccumulator
{
	ImpulseAccumulator()
		: mDeltaX(simd4f(_0)), 
		mDeltaY(mDeltaX), mDeltaZ(mDeltaX), 
		mVelX(mDeltaX), mVelY(mDeltaX), 
		mVelZ(mDeltaX), mNumCollisions(sEpsilon)
	{}

	void add(const Simd4f& x, const Simd4f& y, const Simd4f& z, 
		const Simd4f& scale, const Simd4f& mask)
	{
	...
	void addVelocity(const Simd4f& vx, const Simd4f& vy, const Simd4f& vz,
		const Simd4f& mask)
	{
	...
	void subtract(const Simd4f& x, const Simd4f& y, const Simd4f& z, 
		const Simd4f& scale, const Simd4f& mask)
	{
		PX_ASSERT(allTrue((mask & x)==(mask & x)));
		PX_ASSERT(allTrue((mask & y)==(mask & y)));
...
template <typename Simd4f>
FORCE_INLINE void cloth::SwCollision<Simd4f>::collideSpheres(const Simd4i& sphereMask, 
	const Simd4f* positions, ImpulseAccumulator& accum) const
{
	...
	while(mask)
	{
		...
		if (frictionEnabled)
		{
			...
			accum.addVelocity(splat<0>(velocity), splat<1>(velocity), 
							  splat<2>(velocity), contactMask);
		}
	}
}

template <typename Simd4f>
FORCE_INLINE typename cloth::SwCollision<Simd4f>::Simd4i cloth::SwCollision<Simd4f>::collideCones(
	const Simd4f* __restrict positions, ImpulseAccumulator& accum) const
{
...
template <typename Simd4f>
FORCE_INLINE void cloth::SwCollision<Simd4f>::collideSpheres( const Simd4i& sphereMask, 
	const Simd4f* __restrict prevPos, Simd4f* __restrict curPos, 
	ImpulseAccumulator& accum) const
{
	...
	while(mask)
	{
		...
		Simd4f prevRadius = splat<3>(prevSphere) * sSkeletonWidth;
		...
		Simd4f curRadius = splat<3>(curSphere) * sSkeletonWidth;
		...
		Simd4f collisionMask = (toi < simd4f(_1)) & (halfB < sqrtD); //add parentheses for wiiu
		
		// At least on PSP2 'sqrt(negative) > value' can be true (instead 
		// 'unordered' according to IEEE. Make bullet proof by masking off:
		collisionMask = collisionMask & (discriminant >= simd4f(_0)); //add parentheses for wiiu

		// a is negative when one sphere is contained in the other,
		// which is already handled by discrete collision.
		collisionMask = collisionMask & (minusA < -(Simd4f)sEpsilon); //add parentheses for wiiu
		...
		Simd4f negativeScale = simd4f(_1) - rsqrt(sqrDistance) * splat<3>(curSphere);
		...
		if (frictionEnabled)
		{
			...
			accum.addVelocity(splat<0>(velocity),
							  splat<1>(velocity),
							  splat<2>(velocity),
							  contactMask);
		}
	...
	ImpulseAccumulator& accum) const
{
	...
	while(mask)
	{
		...
		Simd4f prevInnerRadius = prevRadius * sSkeletonWidth;
		Simd4f curInnerRadius = curRadius * sSkeletonWidth;

		Simd4f dotPrevPrev = prevT*prevT + prevU*prevU + prevV*prevV - prevInnerRadius*prevInnerRadius;
		Simd4f dotPrevCur = prevT*curT + prevU*curU + prevV*curV - prevInnerRadius*curInnerRadius;
		Simd4f dotCurCur = curSqrDistance - curInnerRadius*curInnerRadius;
		...
		if(!allEqual(collisionMask, simd4f(_0)))
		{
			...
			Simd4f deltaScaledAxis = curScaledAxis - 
				prevAxis * splat<1>(simd4f(prevAuxiliary));
			...
			Simd4f slope = (prevSlope * oneMinusToi + curSlope * toi) * sSkeletonWidth;
			...
			Simd4f invDistance = rsqrt(sqrDistance) & (sqrDistance > simd4f(_0)); //add parentheses for wiiu

			Simd4f base = dot + slope * sqrDistance * invDistance;
			Simd4f scale = base * invHalfLength & collisionMask;

			Simd4f cullMask = (abs(scale) < simd4f(_1)) & collisionMask; //add parentheses for wiiu
...
template <typename Simd4f>
PX_INLINE void calculateFrictionImpulse(
	const Simd4f& deltaX, const Simd4f& deltaY, const Simd4f& deltaZ, 
	const Simd4f& velX, const Simd4f& velY, const Simd4f& velZ, 
	const Simd4f* curPos, const Simd4f* prevPos, 
    // START ADDED BY NSF-DRESSING
	const Simd4f& scale, const Simd4f& coefficient, const Simd4f& mask, Simd4f* impulse, Simd4f* tangvelo = NULL)
    // END ADDED BY NSF-DRESSING
{
	// calculate collision normal
	Simd4f deltaSq = deltaX*deltaX +
					 deltaY*deltaY + 
					 deltaZ*deltaZ;

    // START ADDED BY NSF-DRESSING

    const float * sq = array(deltaSq);
    float rstsq[4];
    for (int i = 0; i < 4; i++) {
        if (sq[i] == 0) {
            rstsq[i] = FLT_EPSILON;
	...
	Simd4f rcpDelta = rsqrt(simd4f(rstsq[0], rstsq[1], rstsq[2], rstsq[3]));
    // END ADDED BY NSF-DRESSING
    ...
    // START ADDED BY NSF-DRESSING
    /*Simd4f deltan = nx*nx + ny * ny + nz * nz;
    rcpDelta = rsqrt(deltan + sEpsilon);
    ...
    // START ADDED BY NSF-DRESSING
    /*const float * dx = array(rvy), *dy = array(deltaY), *dz = array(ny);
    printf("delta xyz: %f, %f, %f, %f    %f, %f, %f, %f    %f, %f, %f, %f\n", dx[0], dx[1], dx[2], dx[3], dy[0], dy[1], dy[2], dy[3], dz[0], dz[1], dz[2], dz[3]);*/
    // END ADDED BY NSF-DRESSING
    ...
    // START ADDED BY NSF-DRESSING
    if (tangvelo != NULL) {
        tangvelo[0] = rvtx&mask;
        tangvelo[1] = rvty&mask;
        tangvelo[2] = rvtz&mask;
    }
    // END ADDED BY NSF-DRESSING
}
...
void cloth::SwCollision<Simd4f>::collideParticles()
{
	...
	for(; pIt < pEnd; pIt += 16, prevIt += 16)
	{
		...
		if (frictionEnabled)
		{
			...
			calculateFrictionImpulse(accum.mDeltaX,	accum.mDeltaY, accum.mDeltaZ,
				accum.mVelX, accum.mVelY, accum.mVelZ, curPos, prevPos,
				invNumCollisions, frictionScale, mask,frictionImpulse);
		...
		if (massScalingEnabled)
		{
			...
			Simd4f dSq = invNumCollisions*invNumCollisions*
				(accum.mDeltaX*accum.mDeltaX +
				 accum.mDeltaY*accum.mDeltaY + 
				 accum.mDeltaZ*accum.mDeltaZ);
...
void cloth::SwCollision<Simd4f>::collideVirtualParticles()
{
	...
	Simd4f* __restrict dummy = mClothData.mNumParticles
		+ reinterpret_cast<Simd4f*>(mClothData.mCurParticles);
	Simd4f invGridScale = recip(mGridScale) & (mGridScale > sEpsilon); //add parentheses for wiiu
	...
	for (; vpIt != vpEnd; vpIt += 16)
	{
		...
		if (frictionEnabled)
		{
            ...
            // modify current position using friction impulse
            p0v0 = p0v0 + (splat<0>(rw0) * frictionImpulse[0]);
            p0v1 = p0v1 + (splat<1>(rw0) * frictionImpulse[0]);
            p0v2 = p0v2 + (splat<2>(rw0) * frictionImpulse[0]);
            
            p1v0 = p1v0 + (splat<0>(rw1) * frictionImpulse[1]);
            p1v1 = p1v1 + (splat<1>(rw1) * frictionImpulse[1]);
            p1v2 = p1v2 + (splat<2>(rw1) * frictionImpulse[1]);
            
            p2v0 = p2v0 + (splat<0>(rw2) * frictionImpulse[2]);
            p2v1 = p2v1 + (splat<1>(rw2) * frictionImpulse[2]);
            p2v2 = p2v2 + (splat<2>(rw2) * frictionImpulse[2]);
            
            p3v0 = p3v0 + (splat<0>(rw3) * frictionImpulse[3]);
            p3v1 = p3v1 + (splat<1>(rw3) * frictionImpulse[3]);
            p3v2 = p3v2 + (splat<2>(rw3) * frictionImpulse[3]);
            // END ADDED BY NSF-DRESSING
...
template <typename Simd4f>
// START ADDED BY NSF-DRESSING
void cloth::SwCollision<Simd4f>::collideContinuousParticles(float iterDt)
// END ADDED BY NSF-DRESSING
{
	Simd4f curPos[4];
	Simd4f prevPos[4];
	...
	for(; curIt < curEnd; curIt += 16, prevIt += 16)
	{
        ...
        // START ADDED BY NSF-DRESSING
        Simd4f cone_numcol = Simd4f(accum.mNumCollisions);
        // END ADDED BY NSF-DRESSING
		collideSpheres(sphereMask, prevPos, curPos, accum);
        // START ADDED BY NSF-DRESSING
        Simd4f spherecollide = simd4f(1.0) - (simd4f(1.0) & (cone_numcol == accum.mNumCollisions));
        spherecollide = spherecollide * simd4f(0.0);
        // END ADDED BY NSF-DRESSING
		...
		if (frictionEnabled)
		{
			Simd4f frictionImpulse[3];
            // START ADDED BY NSF-DRESSING
            Simd4f tangentialVelocity[3];
            
            Simd4f mmask;
            Simd4f temp_frictionScale = frictionScale;
            if (anyGreater(simd4f(-0.25), curPos[0], mmask)) {
                temp_frictionScale = temp_frictionScale + (temp_frictionScale&mmask) * simd4f(0.0);
            }
            
			calculateFrictionImpulse(accum.mDeltaX,	accum.mDeltaY, accum.mDeltaZ,
				accum.mVelX, accum.mVelY, accum.mVelZ, curPos, prevPos,
				invNumCollisions, temp_frictionScale, mask,frictionImpulse, tangentialVelocity);
            ...
            // modify current position using friction impulse
            curPos[0] = curPos[0] + frictionImpulse[0];
            curPos[1] = curPos[1] + frictionImpulse[1];
            curPos[2] = curPos[2] + frictionImpulse[2];
        ...
        // START ADDED BY NSF-DRESSING
        /*const float* contact1 = array(accum.mDeltaX * invNumCollisions),
        * contact2 = array(accum.mDeltaY * invNumCollisions),
        * contact3 = array(accum.mDeltaZ * invNumCollisions);
        mClothData.mAccumulatedCollisionForce[0] += sum3f(contact1);
        mClothData.mAccumulatedCollisionForce[1] += sum3f(contact2);
        mClothData.mAccumulatedCollisionForce[2] += sum3f(contact3);
        for (int v = 0; v < 4; v++) {
            mClothData.mContactForces[(vert_index+v) * 3] += contact1[v];
        }
        for (int v = 0; v < 4; v++) {
            mClothData.mContactForces[(vert_index+v) * 3 + 1] += contact2[v];
        }
        for (int v = 0; v < 4; v++) {
            mClothData.mContactForces[(vert_index+v) * 3 + 2] += contact3[v];
        }*/
...
void cloth::SwCollision<Simd4f>::collideConvexes(const IterationState<Simd4f>& state)
{
	...
	Simd4f* planes = static_cast<Simd4f*>(mAllocator.allocate(
		sizeof(Simd4f) * mClothData.mNumPlanes * 2));

	const Simd4f* targetPlanes = reinterpret_cast<
		const Simd4f*>(mClothData.mTargetCollisionPlanes);

	// generate plane collision data
	if (state.mRemainingIterations != 1)
	{
		// interpolate planes
		LerpIterator<Simd4f, const Simd4f*> planeIter(reinterpret_cast<const Simd4f*>(
			mClothData.mStartCollisionPlanes), targetPlanes, state.getCurrentAlpha());
	...
	float* __restrict prevIt = mClothData.mPrevParticles;
    // START ADDED BY NSF-DRESSING
    // initialize to zero accumulated force
    int vert_index = 0;
    // END ADDED BY NSF-DRESSING
	for(; curIt < curEnd; curIt += 16, prevIt += 16)
	{
		...
		if (frictionEnabled)
		{
			...
			calculateFrictionImpulse(accum.mDeltaX,	accum.mDeltaY, accum.mDeltaZ,
				accum.mVelX, accum.mVelY, accum.mVelZ, curPos, prevPos,
                // START ADDED BY NSF-DRESSING
				invNumCollisions, frictionScale, mask, frictionImpulse, tangentialVelocity);
                // END ADDED BY NSF-DRESSING
...
template <typename Simd4f>
void cloth::SwCollision<Simd4f>::collideConvexes(const Simd4f* __restrict planes, 
	Simd4f* __restrict curPos, ImpulseAccumulator& accum)
{
	...
	{
		*dIt = splat<3>(*pIt) + curPos[2] * splat<2>(*pIt) + 
			curPos[1] * splat<1>(*pIt) + curPos[0] * splat<0>(*pIt);
...
void cloth::SwCollision<Simd4f>::collideTriangles(const IterationState<Simd4f>& state)
{
    ...
    // START ADDED BY NSF-DRESSING
    printf("collide triangles1\n");
    // END ADDED BY NSF-DRESSING
	TriangleData* triangles = static_cast<TriangleData*>(mAllocator.allocate(
		sizeof(TriangleData) * mClothData.mNumTriangles));
	...
	if (state.mRemainingIterations != 1)
	{
		...
		LerpIterator<Simd4f, UnalignedIterator<Simd4f, 3> > triangleIter(
			mClothData.mStartCollisionTriangles, targetTriangles, state.getCurrentAlpha());
...
template <typename Simd4f>
void cloth::SwCollision<Simd4f>::collideTriangles(const TriangleData* __restrict triangles, 
	Simd4f* __restrict curPos, ImpulseAccumulator& accum)
{
    // START ADDED BY NSF-DRESSING
    printf("collide triangles2\n");
    // END ADDED BY NSF-DRESSING
	...
	for(tIt = triangles; tIt != tEnd; ++tIt)
	{
		...
		Simd4f numerator = edge1SqrLength - edge0DotEdge1 
			+ deltaDotEdge0 - deltaDotEdge1;
