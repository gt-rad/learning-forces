...
{
	return splat<0>(vec) * columns[0]
		+  splat<1>(vec) * columns[1]
		+  splat<2>(vec) * columns[2];
}

template <typename Simd4f>
inline Simd4f transform(const Simd4f (&columns)[3], 
	const Simd4f& translate, const Simd4f& vec)
{
	return translate + splat<0>(vec)*columns[0] + 
		splat<1>(vec)*columns[1] + splat<2>(vec)*columns[2];
}

template <typename> struct IterationState; // forward declaration
...
struct IterationStateFactory
{
	...
	{
		return *reinterpret_cast<const PxVec3*>(
...
cloth::IterationStateFactory::IterationStateFactory(MyCloth& cloth, float frameDt)
{
	...
	mIterDtAverage = cloth.mIterDtAvg.empty() ? 
		mIterDt : cloth.mIterDtAvg.average();
	...
	cloth.mLinearVelocity = invFrameDt * 
		(cloth.mTargetMotion.p - cloth.mCurrentMotion.p);
...
cloth::IterationState<Simd4f> cloth::IterationStateFactory::create(MyCloth const& cloth) const
{
	...
	Simd4f gravity = load(array(cloth.mGravity))
		* (Simd4f)simd4f(sqr(mIterDtAverage));
	...
	Simd4f linearDrag = (simd4f(_1) - simdf::exp2(load(
		array(cloth.mLinearLogDrag)) * dampExponent))
		* iterDt * curLinearVelocity;

	// portion of frame acceleration to transfer to particle
	Simd4f linearInertia = load(array(cloth.mLinearInertia)) 
		* iterDt * (prevLinearVelocity - curLinearVelocity);
	...
	result.mIsTurning = mPrevAngularVelocity.magnitudeSquared() 
		+ cloth.mAngularVelocity.magnitudeSquared() > 0.0f;

	if(result.mIsTurning)
	{
		Simd4f curAngularVelocity = load(array(
			invRotation.rotate(cloth.mAngularVelocity)));
		Simd4f prevAngularVelocity = load(array(
			invRotation.rotate(mPrevAngularVelocity)));
		...
		Simd4f angularDrag = simd4f(_1) - simdf::exp2(load(
			array(cloth.mAngularLogDrag)) * dampExponent);
		...
		float dragScale = array(
			rsqrt(velocityLengthSqr * dragLengthSqr) * dragLengthSqr)[0];
		float inertiaScale = mInvNumIterations * array(
			rsqrt(accelerationLengthSqr * inertiaLengthSqr) * inertiaLengthSqr)[0];
		...
		float centrifugalScale = centrifugalAlpha * array(
			rsqrt(velocityLengthSqr * centrifugalLengthSqr) * centrifugalLengthSqr)[0] - inertiaScale;
		...
		for(unsigned int j=0; j<3; ++j)
		{
			...
			for(unsigned int i=0; i<3; ++i)
			{
				...
				prevMatrix(i, j) = centrifugal - coriolis
					+ curMatrix(i, j) * (inertiaScale - damping)
					- prevMatrix(i, j) * inertiaScale;
				curMatrix(i, j) = centrifugal + coriolis
					+ curMatrix(i, j) * (inertiaScale + damping + dragScale);
			}
