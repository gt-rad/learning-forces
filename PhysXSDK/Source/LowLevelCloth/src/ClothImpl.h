...
public:
    ...
    virtual PxVec3 getAccumulatedCollisionForces() const;
    virtual float* getContactForces() const;
    virtual float* getFrictionForces() const;
    ...
    // START ADDED BY NSF-DRESSING
    virtual void setSelfFriction(float friction);
    virtual float getSelfFriction() const;
    
    virtual void setStiffnessPower(uint32_t power);
    virtual uint32_t getStiffnessPower() const;
    // END ADDED BY NSF-DRESSING
    ...
    // START ADDED BY NSF-DRESSING
    virtual void setSolverFlags(char);
    // END ADDED BY NSF-DRESSING
...
inline void ClothImpl<T>::setSpheres( Range<const PxVec4> spheres, uint32_t first, uint32_t last )
{
	...
	{
		if(PxMax(oldSize, newSize) > PxMin(mCloth.mStartCollisionSpheres.capacity(), 
			mCloth.mTargetCollisionSpheres.capacity()))
		{
			ContextLockType contextLock(mCloth.mFactory);
...
inline void ClothImpl<T>::setPlanes(Range<const PxVec4> planes, uint32_t first, uint32_t last)
{
	...
	{
		if(PxMax(oldSize, newSize) > PxMin(mCloth.mStartCollisionPlanes.capacity(), 
			mCloth.mTargetCollisionPlanes.capacity()))
		{
			ContextLockType contextLock(mCloth.mFactory);
...
inline void ClothImpl<T>::setTriangles(Range<const PxVec3> triangles, uint32_t first, uint32_t last)
{
	...
	{
		if(PxMax(oldSize, newSize) > PxMin(mCloth.mStartCollisionTriangles.capacity(), 
			mCloth.mTargetCollisionTriangles.capacity()))
		{
			ContextLockType contextLock(mCloth.mFactory);
...
template <typename T>
inline void ClothImpl<T>::setTriangles(Range<const PxVec3> startTriangles, Range<const PxVec3> targetTriangles, uint32_t first)
{
	...
	if(newSize > PxMin(mCloth.mStartCollisionTriangles.capacity(), 
		mCloth.mTargetCollisionTriangles.capacity()))
	{
		ContextLockType contextLock(mCloth.mFactory);
		mCloth.mStartCollisionTriangles.reserve(newSize);
...
// START ADDED BY NSF-DRESSING
template <typename T>
inline void ClothImpl<T>::setSelfFriction(float friction)
{
    mCloth.mSelfFriction = friction;
    mCloth.wakeUp();
}

template <typename T>
inline float ClothImpl<T>::getSelfFriction() const
{
    return mCloth.mSelfFriction;
}

template <typename T>
inline void ClothImpl<T>::setStiffnessPower(uint32_t power)
{
    mCloth.mStiffPower = power;
    mCloth.wakeUp();
}

template <typename T>
inline uint32_t ClothImpl<T>::getStiffnessPower() const
{
    return mCloth.mStiffPower;
}
// END ADDED BY NSF-DRESSING
...
inline void ClothImpl<T>::clearInterpolation()
{
	...
	{
		shdfnd::swap(mCloth.mStartCollisionSpheres, mCloth.mTargetCollisionSpheres);
...
{
	return MappedRange<U>(data, data + getNumParticles(), *this, 
		&Cloth::lockParticles, &Cloth::unlockParticles);
}
