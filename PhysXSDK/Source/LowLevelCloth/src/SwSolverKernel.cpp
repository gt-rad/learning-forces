...
namespace avx
{
	...
	template <bool, uint32_t> void solveConstraints(float* __restrict, const float* __restrict, 
		const float* __restrict, const uint16_t* __restrict, const __m128&);
}
...
#if _MSC_FULL_VER < 160040219 || !defined(_XCR_XFEATURE_ENABLED_MASK) // need at least VC10 SP1 and compile on at least Win7 SP1
		return 0;
#else
		int cpuInfo[4];
		__cpuid(cpuInfo, 1);
...
#pragma diag_pop
	...
	void integrateParticles(Simd4f* __restrict curIt, 
		Simd4f* __restrict curEnd, Simd4f* __restrict prevIt, 
		Simd4f scale, const AccelerationIterator& aIt, const Simd4f& prevBias)
	{		
	...
	void integrateParticles(Simd4f* __restrict curIt, 
		Simd4f* __restrict curEnd, Simd4f* __restrict prevIt, 
		const Simd4f (&prevMatrix)[3], const Simd4f (&curMatrix)[3],
		const AccelerationIterator& aIt, const Simd4f& prevBias)
	{
		...
		for(; curIt != curEnd; ++curIt, ++prevIt, ++accelIt)
		{
			...
			Simd4f delta = cloth::transform(curMatrix, cloth::transform(
				prevMatrix, *accelIt, previous), current);
			*curIt = current + (delta & finiteMass);
			*prevIt = select(sMaskW, previous, current) + (prevBias & finiteMass);
		}
	}
	...
	void constrainMotion(Simd4f* __restrict curIt, const Simd4f* __restrict curEnd,
		const ConstraintIterator& spheres, Simd4f scaleBiasStiffness)
	{
		...
		for(; curIt < curEnd; curIt += 4)
		{
			...
			Simd4f delta0 = *sphIt - (sMaskXYZ & curPos0); ++sphIt;
			Simd4f delta1 = *sphIt - (sMaskXYZ & curPos1); ++sphIt;
			Simd4f delta2 = *sphIt - (sMaskXYZ & curPos2); ++sphIt;
			Simd4f delta3 = *sphIt - (sMaskXYZ & curPos3); ++sphIt;

			Simd4f deltaX = delta0, deltaY = delta1, 
				deltaZ = delta2, deltaW = delta3;
	...
	template <typename Simd4f, typename ConstraintIterator>
	void constrainSeparation(Simd4f* __restrict curIt, 
		const Simd4f* __restrict curEnd, const ConstraintIterator& spheres)
	{
		// local copy of iterator to maintain alignment
		ConstraintIterator sphIt = spheres;
	...
	void solveConstraints(float* __restrict posIt, const float* __restrict rIt, 
		const float* __restrict rEnd, const uint16_t* __restrict iIt, Simd4f stiffness, unsigned int stiff_power = 0)
        // END ADDED BY NSF-DRESSING
	{
		...
		for(; rIt != rEnd; rIt += 4, iIt += 8)
		{
            ...
            // START ADDED BY NSF-DRESSING
            // adjust stiffness
            /*Simd4f stretchness = sqrt(e2ij) /(rij + sEpsilon);
            const float* stretch = array(stretchness);
            
            const float* cur_stiff = array(stiffness);
            float mult[4] = {1, 1, 1, 1};
            for (int id = 0; id < 4; id++) {
                if (stretch[id] > 1 && stretch[id] < 2) {
                    mult[id] = 1;
                } else if (stretch[id] >= 2) {
                    mult[id] = stretch[id]*stretch[id]*stretch[id]*stretch[id]*stretch[id]*stretch[id];
                } else {
                    mult[id] = 1.0;
                }
            }
            
            Simd4f new_stiffness = min(simd4f(cur_stiff[0]*mult[0], cur_stiff[1]*mult[1], cur_stiff[2]*mult[2], cur_stiff[3]*mult[3]), simd4f(1));*/
			
            Simd4f stretchness = max(sqrt(e2ij) /(rij + sEpsilon), simd4f(1));
            Simd4f stiff_mult = simd4f(1);
            for (unsigned int s = 0; s < stiff_power; s++) {
                stiff_mult = stiff_mult * stretchness;
            }
            
            Simd4f new_stiffness = min(stiffness * (stiff_mult), simd4f(1));
            // END ADDED BY NSF-DRESSING
            ...
            // START ADDED BY NSF-DRESSING
			// Simd4f exij = erij  * new_stiffness * recip(sEpsilon + vwij);
            // END ADDED BY NSF-DRESSING
...
#include "xbox360\SwSolveContraints.h"
#elif defined(PX_PS3)
#include "ps3\SwSolveContraints.h"
#endif

	// calculates upper bound of all position deltas
	template <typename Simd4f>
	Simd4f calculateMaxDelta(const Simd4f* prevIt, 
		const Simd4f* curIt, const Simd4f* curEnd)
	{
...
cloth::SwSolverKernel<Simd4f>::SwSolverKernel(SwCloth const& cloth, SwClothData& clothData, 
	SwKernelAllocator& allocator, IterationStateFactory& factory, PxProfileZone* profiler)
: mCloth(cloth),
...
void cloth::SwSolverKernel<Simd4f>::integrateParticles(AccelerationIterator& accelIt, const Simd4f& prevBias)
{
	...
	if(!mState.mIsTurning)
		::integrateParticles(curIt, curEnd, prevIt, 
			mState.mPrevMatrix[0], accelIt, prevBias);
	else
		::integrateParticles(curIt, curEnd, prevIt, 
			mState.mPrevMatrix, mState.mCurMatrix, accelIt, prevBias);
}
...
void cloth::SwSolverKernel<Simd4f>::constrainTether()
{
	...
	Simd4f stiffness = (Simd4f)sMaskXYZ & simd4f(numParticles * 
		mClothData.mTetherConstraintStiffness / numTethers);
	Simd4f scale = simd4f(mClothData.mTetherConstraintScale);
    // START ADDED BY NSF-DRESSING
    /*TetherIter titer = tFirst;
    for (; titer < tEnd; titer++) {
        printf("tether %d, length %f\n", titer->mAnchor, titer->mLength);
    }*/
    // END ADDED BY NSF-DRESSING
	for(; curIt != curEnd; curIt+=4, ++tFirst)
	{
        ...
        // START ADDED BY NSF-DRESSING
        const float* pos_arr = array(position);
        float invweight = pos_arr[3];
        // END ADDED BY NSF-DRESSING
		...
		for(TetherIter tIt = tFirst; tIt < tEnd; tIt += numParticles)
		{
            ...
            // START ADDED BY NSF-DRESSING
            const float* anc_arr = array(anchor);
            float anc_invw = anc_arr[3];
            float ratio = 1;
            if (anc_invw == 0) {
                ratio = 1;
            } else {
//                ratio = 1.0/invweight / ((1.0/invweight) + (1.0/anc_invw));
            }
            
			offset = offset + delta * max(slack, simd4f(_0)) * simd4f(ratio);
            
            if (anc_invw != 0) {
                const float* offset_arr = array(offset);
                for (uint16_t iDim = 0; iDim < 3; iDim++) {
                    mClothData.mFrictionForces[tIt->mAnchor * 3 + iDim] += -offset_arr[iDim] / invweight * anc_invw*0;
                }
            }
                
            //storeAligned(mClothData.mCurParticles, tIt->mAnchor * sizeof(PxVec4), anchor - delta * max(slack, simd4f(_0)) * simd4f(1-ratio));
            // END ADDED BY NSF-DRESSING
		}
...
void cloth::SwSolverKernel<Simd4f>::solveFabric()
{
	...
	for(; cIt != cEnd; ++cIt)
	{
        ...
        // START ADDED BY NSF-DRESSING
        uint32_t stiffpower = mClothData.mStiffPower;
        // END ADDED BY NSF-DRESSING
...
#if _MSC_VER >= 1700
			neutralMultiplier
				? avx::solveConstraints<false, 2>(pIt, rIt, rEnd, iIt, stiffness, stiffpower)
				: avx::solveConstraints<true, 2>(pIt, rIt, rEnd, iIt, stiffness, stiffpower);
			break;
#endif
		case 1:
			neutralMultiplier
				? avx::solveConstraints<false, 1>(pIt, rIt, rEnd, iIt, stiffness, stiffpower)
				: avx::solveConstraints<true, 1>(pIt, rIt, rEnd, iIt, stiffness, stiffpower);
			break;
		default:
#endif
			neutralMultiplier
				? solveConstraints<false>(pIt, rIt, rEnd, iIt, stiffness, stiffpower)
				: solveConstraints<true>(pIt, rIt, rEnd, iIt, stiffness, stiffpower);
                // END ADDED BY NSF-DRESSING
#ifdef PX_AVX
			break;
		}
...
void cloth::SwSolverKernel<Simd4f>::iterateCloth()
{
    ...
    // START ADDED BY NSF-DRESSING
    //std::cout << "hi";
    if (mCloth.mSolverFlags & 1) {
        // clear the contact force if integration happens
        // initialize force calculator to zero
        memset(mClothData.mContactForces, 0, sizeof(float) * mClothData.mNumParticles * 3);
        memset(mClothData.mFrictionForces, 0, sizeof(float) * mClothData.mNumParticles * 3);
    ...
    if (mCloth.mSolverFlags & 2) {
        // motion constraints
        constrainMotion();
//std::cout << "2";
    }

    if (mCloth.mSolverFlags & 4) {
        // solve tether constraints
        constrainTether();
//std::cout << "3";
    }

    if (mCloth.mSolverFlags & 8) {
