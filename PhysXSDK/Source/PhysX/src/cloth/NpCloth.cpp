...
void NpCloth::setSelfFrictionCoefficient(PxReal frictionCoefficient)
{
    NP_WRITE_CHECK(NpActor::getOwnerScene(*this));
    
    PX_CHECK_AND_RETURN(frictionCoefficient >= 0.0f || frictionCoefficient <= 1.0f, "PxCloth::setFrictionCoefficient: self friction coefficient has to be between 0 and 1!");
    
    mCloth.setSelfFrictionCoefficient(frictionCoefficient);
    sendPvdSimpleProperties();
}

PxReal NpCloth::getSelfFrictionCoefficient() const
{
    return mCloth.getSelfFrictionCoefficient();
}

void NpCloth::setStiffnessPower(uint32_t stiffpower)
{
    NP_WRITE_CHECK(NpActor::getOwnerScene(*this));
    
    PX_CHECK_AND_RETURN(stiffpower <= 10, "PxCloth::StiffnessPower: StiffnessPower coefficient has to be between 0 and 10!");
    
    mCloth.setStiffnessPower(stiffpower);
    sendPvdSimpleProperties();
}

uint32_t NpCloth::getStiffnessPower() const
{
    return mCloth.getStiffnessPower();
}
...
void NpCloth::setClothFlags(PxClothFlags flags)
{
	...
	for (int i=0; i < PxClothFlag::eCOUNT; ++i)
	{
		PxClothFlag::Enum flag = (PxClothFlag::Enum)(1<<i);
		mCloth.setClothFlag(flag, flags.isSet(flag));
	}
...
// START ADDED BY NSF-DRESSING
PxVec3 NpCloth::getAccumulatedCollisionForce() const {
    return mCloth.getAccumulatedCollisionForce();
}

float* NpCloth::getContactForces() const {
    return mCloth.getContactForces();
}

float* NpCloth::getFrictionForces() const {
    return mCloth.getFrictionForces();
}

void NpCloth::setSolverFlags(char flag) const {
    mCloth.setSolverFlag(flag);
}
// END ADDED BY NSF-DRESSING
...
void NpCloth::visualize(Cm::RenderOutput& out, NpScene* scene)
{
	...
	PxClothFabric* fabric = getFabric();
...
namespace 
{
	Pvd::SceneVisualDebugger* getVisualDebugger( Scb::Scene* scene )
	{
		if ( scene )
		{
			Pvd::SceneVisualDebugger& debugger( scene->getSceneVisualDebugger() );
			if ( debugger.isConnectedAndSendingDebugInformation() )
				return &debugger;
		}
		return NULL;
	}
...
{
#if PX_SUPPORT_VISUAL_DEBUGGER			
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );
	if ( debugger ) debugger->sendSimpleProperties( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER						
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );					
	if ( debugger ) debugger->sendMotionConstraints( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER						
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );					
	if ( debugger ) debugger->sendSelfCollisionIndices( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER						
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );					
	if ( debugger ) debugger->sendRestPositions( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER						
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );					
    if ( debugger ) debugger->sendSeparationConstraints( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER						
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );					
	if ( debugger ) debugger->sendCollisionSpheres( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER						
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );					
	if ( debugger ) debugger->sendCollisionCapsules( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );					
	if ( debugger ) debugger->sendCollisionTriangles( &mCloth );
#endif
}
...
#if PX_SUPPORT_VISUAL_DEBUGGER						
	Pvd::SceneVisualDebugger* debugger = getVisualDebugger( mCloth.getScbSceneForAPI() );						
	if ( debugger ) debugger->sendVirtualParticles( &mCloth );
#endif
}
