...
//~PX_SERIALIZATION
    ...
    // START ADDED BY NSF-DRESSING
    PX_INLINE PxReal					getSelfFrictionCoefficient() const;
    PX_INLINE void						setSelfFrictionCoefficient(PxReal frictionCoefficient);
    
    PX_INLINE unsigned int				getStiffnessPower() const;
    PX_INLINE void						setStiffnessPower(unsigned int);
    // END ADDED BY NSF-DRESSING
	...
	PX_INLINE void						setClothFlag(PxClothFlag::Enum flag, bool val);
...
// START ADDED BY NSF-DRESSING
    PxVec3 getAccumulatedCollisionForce() const {
        return mCloth.getAccumulatedCollisionForce();
    }
    
    float* getContactForces() const {
        return mCloth.getContactForces();
    }
    
    float* getFrictionForces() const {
        return mCloth.getFrictionForces();
    }
    
    void setSolverFlag(char flag) const {
        mCloth.setSolverFlags(flag);
    }
// END ADDED BY NSF-DRESSING
...
// START ADDED BY NSF-DRESSING
PX_INLINE PxReal Cloth::getSelfFrictionCoefficient() const
{
    if (!isBuffering())
    return mCloth.getSelfFrictionCoefficient();
    else
    {
        Ps::getFoundation().error(PxErrorCode::eINVALID_OPERATION, __FILE__, __LINE__, "Call to PxCloth::getSelfFrictionCoefficient() not allowed while simulation is running.");
        return 0.0f;
    }
}


PX_INLINE void Cloth::setSelfFrictionCoefficient(PxReal frictionCoefficient)
{
    if (!isBuffering())
    mCloth.setSelfFrictionCoefficient(frictionCoefficient);
    else
    Ps::getFoundation().error(PxErrorCode::eINVALID_OPERATION, __FILE__, __LINE__, "Call to PxCloth::setSelfFrictionCoefficient() not allowed while simulation is running.");
}

PX_INLINE unsigned int Cloth::getStiffnessPower() const
{
    if (!isBuffering())
    return mCloth.getStiffnessPower();
    else
    {
        Ps::getFoundation().error(PxErrorCode::eINVALID_OPERATION, __FILE__, __LINE__, "Call to PxCloth::getStiffnessPower() not allowed while simulation is running.");
        return 0;
    }
}


PX_INLINE void Cloth::setStiffnessPower(unsigned int stiffpower)
{
    if (!isBuffering())
    mCloth.setStiffnessPower(stiffpower);
    else
    Ps::getFoundation().error(PxErrorCode::eINVALID_OPERATION, __FILE__, __LINE__, "Call to PxCloth::setStiffnessPower() not allowed while simulation is running.");
}
// END ADDED BY NSF-DRESSING
...
PX_INLINE void Cloth::setClothFlag(PxClothFlag::Enum flag, bool val)
{
	if (!isBuffering())
