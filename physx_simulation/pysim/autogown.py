...
Launch 8 gown simulations which each perform 100 simulations:
    python autogown.py 100

Launch one gown simulation which performs 10 simulations:
    python autogown.py -run 10
'''
...
class AutoGown:
    ...
    def varySimulation(self):
        ...
        self.armPosition = [self.random.uniform(-0.05, 0.05), self.random.uniform(-0.45, -0.05), self.random.uniform(-0.1, 0.1)]
        ...
        self.velocity = self.random.uniform(1.0, 1.5)
        ...
        # Spline trajectories
        self.splines = []
        splineCount = int(round(6 * self.velocity))
        ...
        for i in xrange(1, splineCount + 1):
            ...
            self.splines.append([-0.75 * self.velocity * i / splineCount + self.random.uniform(-0.03, 0.03), self.random.uniform(-0.02, 0.02), (-1)**(i+side) * self.random.uniform(0, 0.05), 0, 1, 0, 0])
    ...
    def saveData(self):
        data = {'simTime': self.simTime, 'simSteps': self.simSteps, 'armPosition': self.armPosition, 'rotateFist': self.rotateFist,
                # 'armRotation': self.armRotation, 'initForearmRotation': self.initForearmRotation, 'self.initArmRotation': self.initArmRotation, 'splines': self.splines,
                'velocity': self.velocity, 'armSpheres': self.armSpheres, 'windForce': self.windForce,
        ...
        if not util.saveData(datetime.now().strftime('%H-%M-%S_%f'), data, directory=datetime.now().strftime('%Y-%m-%d_arm'), parentDir=util.rawDataDir):
            # Filename already exists. Try again.
            util.saveData(datetime.now().strftime('%H-%M-%S_%f'), data, directory=datetime.now().strftime('%Y-%m-%d_arm'), parentDir=util.rawDataDir)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '-run':
        autogown = AutoGown(iterations=int(sys.argv[2])) if len(sys.argv) > 2 else AutoGown(iterations=1)
        autogown.runSimulation()
        gc.collect()
        sys.exit()
    else:
        for i in range(36):
            subprocess.call(['nohup python autogown.py -run %s &' % sys.argv[1] if len(sys.argv) > 1 else '10'], shell=True)
            time.sleep(0.1)
