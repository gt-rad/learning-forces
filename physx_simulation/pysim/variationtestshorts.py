...
class VariationTestShorts:
    ...
    def initialize(self, armPosition=[0, 0, 0], velocityFactor=1.5, rotateFist=[0, 0, 1, -np.radians(45)], rotateArm=[0, 0, 0, 0], splines=None):
        '''
        This should be called after the simulator is initialized or reset.
        '''
        self.initData()
        self.varySimulation(armPosition, velocityFactor, rotateFist, rotateArm, splines)
    ...
    def varySimulation(self, armPos=[0, 0, 0], velocityFactor=1.5, rotateFist=[0, 0, 1, -np.radians(45)], rotateArm=[0, 0, 0, 0], splines=None):
        self.velocityFactor = velocityFactor
        self.rotateFist = rotateFist
        self.rotateArm = rotateArm
        self.simulator.positionArm(armPos[0], armPos[1], armPos[2], rotateFist=rotateFist, rotateArm=rotateArm)
        ...
        if splines is not None:
            ...
            for i in xrange(1, len(splines) + 1):
                ...
                splines[i - 1, 0] += -0.7 * velocityFactor * i / splineCount
    ...
    def recordData(self):
        ...
        if self.simStep % 10 == 0:
            ...
            self.points, y = self.labelFunction([[forcemap]], np.array(self.simData['armSpheres']), isArm=False, retPoints=True, retActivations=True, rotateFist=self.rotateFist, rotateArm=self.rotateArm, precomputedPoints=self.points)
    ...
    def fullSequence(self, i, armPosition, velocity, rotateFist, rotateArm, splines, name):
        # Reset simulation and set variation
        self.simulator.reset()
        self.initialize(armPosition=armPosition, velocityFactor=velocity, rotateFist=[0, 0, 1, -np.radians(45)], rotateArm=rotateArm, splines=splines)
    ...
    def runSimulation(self):
        for i, (armPosition, splines) in enumerate(zip(self.armPositions, self.splines)):
            for velocity in velocities:
                self.fullSequence(i, armPosition, velocity, [0, 0, 1, -np.radians(45)], [0, 0, 0, 0], np.array(splines), 'velocity_%.2f' % velocity)

            for rotation in rotateX:
                self.fullSequence(i, armPosition, 1.75, [0, 0, 1, -np.radians(45)], [1, 0, 0, rotation], np.array(splines), 'rotateX_%.2f' % rotation)

            for rotation in rotateY:
                self.fullSequence(i, armPosition, 1.75, [0, 0, 1, -np.radians(45)], [0, 1, 0, rotation], np.array(splines), 'rotateY_%.2f' % rotation)

            for rotation in rotateZ:
                self.fullSequence(i, armPosition, 1.75, [0, 0, 1, -np.radians(45)], [0, 0, 1, rotation], np.array(splines), 'rotateZ_%.2f' % rotation)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '-run':
        ...
        variationtestshorts = VariationTestShorts(armPositions=allArmPositions[posIndex:posIndex+posCount], splines=allSplines[posIndex:posIndex+posCount], saveDir=sys.argv[3], processId=procId, modelDir='2016-09-11_04-41-18_03-49-00_ForceTorqueVelocity_rawPressuremap_neoleg', epoch=9)
        variationtestshorts.runSimulation()
        gc.collect()
        sys.exit()
    else:
        directory = datetime.now().strftime('%Y-%m-%d_%H-%M-%S_ForceTorqueVelocity_rawPressuremap_neoleg_rotation')
        processCount = 16
        for i in range(processCount):
            subprocess.call(['nohup python variationtestshorts.py -run %d %s %d &' % (i, directory, processCount)], shell=True)
            time.sleep(1)
