...
string logFilePath = "../cmaSearch.log";
/* --------------------------------------------------------- */
/* ------------------- Declarations ------------------------ */
/* --------------------------------------------------------- */

void setCMALogFilePath(const string& filePath)
{
...
cmaes_GetPtr( cmaes_t *t, char const *s)
{
    ...
    /* mean of the recent distribution */
        ...
        return(t->rgxmean);
    else if (strncmp(s, "fbestever", 9) == 0)
    {
        return &(t->rgxbestever[N]); /* f-value */
    }
            ...
            if((nb=sscanf(s, keys[ikey], sin1, sin2, sin3, sin4)) >= 1)
            {
                switch(ikey) {
                    ...
                    case 2 : /* "write" */
                        ...
                        // jie tan hack to change the name of resume write
                    {
                        strcpy(sin2, logFilePath.c_str());
                        
                        sin2[strlen(sin2) - 1] = 't';
                        sin2[strlen(sin2) - 2] = 'a';
                        sin2[strlen(sin2) - 3] = 'd';
...
readpara_SupplementDefaults(readpara_t *t)
{
    ...
    if (t->seed < 1) {
        while ((int) (cloc - clock()) == 0)
            ; /* TODO: remove this for time critical applications!? */
        t->seed = (unsigned int)abs(long(100 *time(NULL)+clock() & 0xffffffff));
    }
    ...
    if (t->lambda < 2)
        t->lambda = 16;//6+(int)(3*log((double)N));  //need modification tanjie
    if (t->mu == -1) {
        t->mu = t->lambda/2; 
            ...
            char const *s4)
{
    ...
    cmaes_WriteLogFile(" *** CMA-ES ABORTED, see errcmaes.err *** \n");
                  ...
                  char const *s3, char const *s4)
{
    ...
    {	  
        sprintf(logStr, "\nFATAL ERROR: %s\n", s2 ? szCat(s1, s2, s3, s4) : s1);
        cmaes_WriteLogFile(logStr);
        cmaes_WriteLogFile("cmaes_t could not open file 'errcmaes.err'.");
        cmaes_WriteLogFile("\n *** CMA-ES ABORTED *** ");	
...
void cmaes_WriteLogFile(const char* log)
{
    printf(log);
    //LOG(WARNING) << log;
#ifdef _WIN32
    FILE* fplog = fopen(logFilePath.c_str(), "a");
    if (fplog)
    {
        fprintf(fplog, log);
        fflush(fplog);
