...
void shortsSimulator::initialize(int maxSteps, bool enableWind) {
    ...
    sphereManager->addCapsule(0, 1); // thigh
    sphereManager->addCapsule(1, 2); // leg
    sphereManager->addCapsule(2, 3); // foot
    ...
    Matrix3d rot;
    rot = AngleAxisd(-1.57, Vector3d::UnitY()) * AngleAxisd(0, Vector3d::UnitX()) * AngleAxisd(0, Vector3d::UnitZ());
    cloth.density = 0.145;

    string gown_path;
    DecoConfig::GetSingleton()->GetString("SHORTS", "MeshFilePath", gown_path);
    cloth.loadMesh(string(PHYSX_ROOT_PATH)+gown_path, Vector3d(-forearm_length + 0.75, 0.85, 0.4), rot, Vector3d(0.8, 0.8, 0.8));
    cloth.createCloth(gPhysicsSDK, gPose);

    DecoConfig::GetSingleton()->GetFloat("SHORTS", "VStretch", cloth.vstretch_stiff[0]);
    DecoConfig::GetSingleton()->GetFloat("SHORTS", "HStretch", cloth.hstretch_stiff[0]);
    DecoConfig::GetSingleton()->GetFloat("SHORTS", "Shear", cloth.shear_stiff[0]);
    DecoConfig::GetSingleton()->GetFloat("SHORTS", "Bend", cloth.bend_stiff[0]);
    DecoConfig::GetSingleton()->GetFloat("SHORTS", "Friction", cloth.friction);
    DecoConfig::GetSingleton()->GetFloat("SHORTS", "SelfFriction", cloth.self_friction);
    DecoConfig::GetSingleton()->GetFloat("SHORTS", "SelfCollisionDist", cloth.self_collision_distance);
    DecoConfig::GetSingleton()->GetInt("SHORTS", "Iteration", cloth_solve_iteration_aftergrip);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "Damping", damping_aftergrip);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "HeightOffset", arm_height_perturb);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "FistRadius", fist_radius);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "WristRadius", wrist_radius);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "ElbowRadius", elbow_radius);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "ShoulderRadius", shoulder_radius);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "ForearmLength", forearm_length);
    DecoConfig::GetSingleton()->GetDouble("SHORTS", "UpperArmLength", upperearm_length);
...
void shortsSimulator::positionArm(double horizontalOffset, double verticalOffset, double sidewaysOffset, vector<double> rotateFist, vector<double> rotateArm, vector<double> rotateArmCenter) {
    PxSphereManager* sphereManager = PxSphereManager::GetSingleton();
    ...
    sphereManager->addSphere(PxVec3(-0.846 + horizontalOffset, 0 + verticalOffset, sidewaysOffset), 0.081); // hip
    sphereManager->addSphere(PxVec3(-0.422 + horizontalOffset, 0 + verticalOffset, sidewaysOffset), 0.053); // knee
    sphereManager->addSphere(PxVec3(0 + horizontalOffset, 0 + verticalOffset, sidewaysOffset), 0.048); // ankle
    sphereManager->addSphere(PxVec3(0 + horizontalOffset, 0.18 + verticalOffset, sidewaysOffset), 0.04); // toe
    sphereManager->addSphere(PxVec3(0 + horizontalOffset, -0.015 + verticalOffset, sidewaysOffset), 0.048); // heel

    // sphereManager->addSphere(PxVec3(-fist_radius-0.1+horizontalOffset, arm_height, sidewaysOffset), fist_radius); // fist
    // sphereManager->addSphere(PxVec3(-fist_radius-0.1+horizontalOffset, arm_height, sidewaysOffset), wrist_radius); // wrist
    // sphereManager->addSphere(PxVec3(-forearm_length-elbow_radius-0.1+horizontalOffset, arm_height, sidewaysOffset), elbow_radius); // elbow
    // sphereManager->addSphere(PxVec3(-forearm_length-elbow_radius-0.1+horizontalOffset, arm_height, sidewaysOffset - upperearm_length), shoulder_radius); // shoulder
    ...
    if (rotateFist[3] != 0 && (rotateFist[0] != 0 || rotateFist[1] != 0 || rotateFist[2] != 0)) {
        ...
        sphereManager->rotateAround(3, angle, axis, elbowCenter);
        sphereManager->rotateAround(4, angle, axis, elbowCenter);
    }
    ...
    if (rotateArm[3] != 0 && (rotateArm[0] != 0 || rotateArm[1] != 0 || rotateArm[2] != 0)) {
        ...
        PxVec3 center = sphereManager->getSpheres()[2].getXYZ();
        sphereManager->rotateAround(3, angle, axis, center);
        sphereManager->rotateAround(4, angle, axis, center);
    }
}

void shortsSimulator::initSpline(vector<Eigen::VectorXd> splinePoints) {
	Spline<Eigen::VectorXd> gripper_motion;
...
void shortsSimulator::setVelocityFactor(double velocityFactor) {
    orig_move_step[0] = - moving_speed * gTimeStep;
    move_step[0] = - moving_speed * gTimeStep * velocityFactor;
    velocity = velocityFactor;
}

void shortsSimulator::rotateSphereAround(int index, double angle, vector<double> axis, vector<double> center) {
    PxSphereManager* sphereManager = PxSphereManager::GetSingleton();
...
int shortsSimulator::getParameterSize() {
    return cloth.parameter_size + parameter_size;
}

void shortsSimulator::getParameterBound(VectorXd& min, VectorXd& max) {
...
void shortsSimulator::randomizeWind() {
    if (windEnabled) {
...
void shortsSimulator::simulate(int steps) {
...
void shortsSimulator::updateForces(int steps) {    // update collision forces and solve gripper constraints
    PxSphereManager* sphereManager = PxSphereManager::GetSingleton();
...
void shortsSimulator::identifyGrippedParticles() {
    PxClothParticleData* particle_data = cloth.mCloth->lockParticleData(PxDataAccessFlag::eWRITABLE);
...
// set parameter
void shortsSimulator::setParameter(VectorXd param) {
